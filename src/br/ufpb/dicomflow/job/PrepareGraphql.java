...
public class PrepareGraphql {
	...
	private void insertRegistries(List<Access> accesses) {
		...
		ControllerProperty currentDateProperty = (ControllerProperty) persistentService.select("property", ControllerProperty.GRAPHQL_CURRENT_DATE_PROPERTY, ControllerProperty.class);
		...
		while (studies.next()) {
			...
			GraphqlService serviceDB = (GraphqlService) persistentService.select("studyIuid", study.getStudyIuid(), GraphqlService.class);
			if(serviceDB != null){
				continue;
			}
			...
			GraphqlService service = new GraphqlService(urlGenerator.getURL(study));
			service.setStudyIuid(study.getStudyIuid());
			service.setPatientId(patient.getPatientId());
			service.setStatus(GraphqlService.OPEN);
			...
			while (it2.hasNext()) {
				...
				if(verifyAccess(access, study, ServicePermission.REQUEST_SERVICE)){
					GraphqlServiceAccess ra = new GraphqlServiceAccess(service, access);
			...
			try {
				if(currentDateProperty == null){
					...
					currentDateProperty.setProperty(ControllerProperty.GRAPHQL_CURRENT_DATE_PROPERTY);
				}
