...
public class VerifyCertificateConfirm {
	...
	public void execute() {
		...
		ControllerProperty currentDateProperty = (ControllerProperty) persistentService.select("property", ControllerProperty.CERTIFICATE_CONFIRM_CURRENT_DATE_PROPERTY, ControllerProperty.class);
		...
		try {
			...
			if(strategy.equals(DAILY_STRATEGY)){
				...
				accesses = messageService.getCertificateConfirms(currentDate, null, null);
			...
			if(strategy.equals(INTERVAL_STRATEGY)){
				...
				try{
					...
					accesses = messageService.getCertificateConfirms(startDate, finishDate, null);
			...
			if(strategy.equals(CURRENT_STUDY_STRATEGY)){
				...
				try{
					...
					accesses = messageService.getCertificateConfirms(startDate, null, null);
		...
		try {
			if(currentDateProperty == null){
				...
				currentDateProperty.setProperty(ControllerProperty.CERTIFICATE_CONFIRM_CURRENT_DATE_PROPERTY);
			}
			Date currentDate = Calendar.getInstance().getTime();
