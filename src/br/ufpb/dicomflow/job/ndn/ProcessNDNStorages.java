...
public class ProcessNDNStorages {
	...
	public void execute() {
		...
		while (it.hasNext()) {
			...
			String uri = storageService.getLink();
			...
			try {
				...
				storageService.setProcessAttempt(storageService.getProcessAttempt()+1);
				storageService.setStatus(StorageService.PROCESSED);
			} catch (MalformedURLException e) {
				storageService.setProcessAttempt(storageService.getProcessAttempt()+1);
				storageService.setStatus(StorageService.PENDING_PROCESS);
			...
			try {
				storageService.save();
				Util.getLogger(this).debug("STORED REAL URL");
